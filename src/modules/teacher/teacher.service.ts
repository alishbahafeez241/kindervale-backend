...
export class TeacherService {
  ...
  async getTeachers(query: TeacherListQueryDto) {
    ...
    const [items, [{ total }]] = await Promise.all([
      this.databaseService.db.select().from(teachersTable).where(where).orderBy(orderBy).limit(limit).offset(offset),
  ...
  async getTeacher(id: string): Promise<Teacher> {
    const [teacher] = await this.databaseService.db.select().from(teachersTable).where(eq(teachersTable.id, id)).limit(1);
